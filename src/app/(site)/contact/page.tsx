...
export default function ContactPage() {
  ...
  async function handle() {
    if (!form.name||!form.email||!form.subject||!form.message){toast.error('All fields required.');return;}
    setBusy(true);
    try {
      await Promise.all([
        contactsApi.submit(form),
        emailService.contactForm(form.name,form.email,form.subject,form.message),
      ]);
        ...
        <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:.5}}>
          ...
          {sent ? (
            <div className="bg-green-50 border-2 border-green-300 rounded-2xl p-8 text-center">
              ...
              <button onClick={()=>{setSent(false);setForm({name:'',email:'',subject:'',message:''}); }} className="mt-5 text-sm font-bold text-primary underline">Send Another</button>
            </div>
          ):(
            <div className="bg-white rounded-2xl p-8 border border-[#f0e8d5] shadow-sm space-y-5">
              {(['name','email','subject'] as const).map(k=>(
                <div key={k}>
                  <label className="block text-xs font-extrabold uppercase tracking-wider text-[#9B6E50] mb-2 capitalize">{k}</label>
              ...
              <div>
                ...
                <textarea value={form.message} onChange={e=>setForm(f=>({...f,message:e.target.value}))} rows={5}
                  placeholder="Tell us about your adoption inquiry…"
