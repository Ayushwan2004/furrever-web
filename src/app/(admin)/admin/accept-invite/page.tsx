...
export default function AcceptInvitePage() {
  ...
  useEffect(() => {
    if (!loading && user && fbUser && code && status === 'idle') {
      handleAccept();
    }
  }, [loading, user, fbUser, code]);

  async function handleAccept() {
    if (!code || !user || !fbUser) return;
    setStatus('accepting');
    try {
      const res: any = await invitesApi.accept(code, fbUser.uid);
      if (res?.success) {
        await fbUser.getIdToken(true); // ✅ force-refresh claims
        setStatus('done');
