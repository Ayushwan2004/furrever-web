...
export default function CertificationsPage() {
  ...
  // ✅ Loads the adoption fields AND checks if app already created a cert for this pet
  async function prefillFromAdoption(a: any) {
    ...
    // Read existing cert from shared 'certificates' collection (may have been created by app)
    try {
  ...
  async function handleIssue() {
    ...
    try {
      ...
      if (form.adopterEmail || result?.adopterEmail) {
        try {
          await emailService.certificate(
            form.adopterEmail || result.adopterEmail,
            form.adopterName,
            form.petName,
            result.certId,
            form.message || 'Congratulations on your new companion!',
          );
        } catch {}
      }
      ...
      setForm({
        adopterId: '', adopterName: '', adopterEmail: '',
        petId: '', petName: '', petImage: '', breed: '', category: '', color: '', age: '',
        adoptionId: '', certType: 'adoption', message: '', expiresAt: '',
      });
  ...
  async function handleRevoke(cert: any) {
    ...
    try {
      // ✅ FIXED: pass petId — it is the document ID in the shared 'certificates' collection
      await certsApi.revoke(cert.petId);
  ...
  const columns: Column<any>[] = [
    ...
    },
    {
      ...
      render: (r) => (
        <div className="flex items-center gap-2">
          <Avatar name={r.adopterName} size={28} />
          <span className="font-bold text-sm">{r.adopterName}</span>
        </div>
      ),
    },
    {
      ...
      render: (r) => (
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 rounded-lg overflow-hidden bg-[#f0e8d5]">
            {r.petImage
              ? <img src={r.petImage} className="w-full h-full object-cover" />
    ...
    { key: 'status',       label: 'Status',    render: (r) => <Badge status={r.status || 'active'} /> },
    {
      ...
      render: (r) => (
        <span className="text-xs font-bold text-[#9B6E50]">
          v{r.version || 1}{r.issuedBy === 'system' ? ' 🤖' : ' 👤'}
        </span>
      ),
    },
    ...
    { key: 'issuedByName', label: 'Issued By', render: (r) => <span className="text-xs font-semibold">{r.issuedByName || (r.issuedBy === 'system' ? 'App' : 'Admin')}</span> },
    {
      ...
      render: (r) => (
        <div className="flex gap-1.5">
          ...
          {r.status !== 'revoked' && (
            <ActionBtn onClick={() => handleRevoke(r)} variant="danger">Revoke</ActionBtn>
          )}
        </div>
      ),
    },
  ...
  return (
    <div>
      ...
      <Modal open={!!selected} onClose={() => setSelected(null)} title="Certificate Details" wide>
        {selected && (
          <div>
            <div className="bg-[#fdf4e3] border-2 border-primary/40 rounded-2xl p-6 mb-5 text-center relative overflow-hidden">
            ...
            {selected.message && (
              <div className="mt-4 p-4 bg-[#fdf4e3] rounded-xl text-sm italic text-[#543e35]">"{selected.message}"</div>
            )}
          </div>
        )}
      </Modal>
      ...
      <Modal open={issueModal} onClose={() => { setIssueModal(false); setExistingCert(null); }} title="Issue Certificate" wide>
        <div className="space-y-4">
          ...
          {/* Step 1 — pick adoption */}
          <div>
            <label className="block text-xs font-extrabold uppercase tracking-wider text-[#9B6E50] mb-2">
              Select Approved Adoption *
            </label>
            <select
              onChange={e => { const a = approvedAdoptions.find(a => a.id === e.target.value); if (a) prefillFromAdoption(a); }}
              ...
              {approvedAdoptions.map(a => (
                <option key={a.id} value={a.id}>
                  {a.petName} → {a.adopterName} ({fmtDate(a.createdAt)})
                </option>
              ))}
            </select>
          </div>
          ...
          {form.petName && (
            <div className="p-3.5 bg-green-50 border border-green-200 rounded-xl text-xs font-semibold text-green-700 flex items-center gap-2">
              ✓ Pre-filled: {form.petName} → {form.adopterName} ({form.adopterEmail || 'no email'})
            </div>
          )}

          {/* ✅ App-cert banner — warns admin when app already created one */}
          {existingCert && (
            <div className="p-3.5 bg-amber-50 border border-amber-300 rounded-xl text-xs font-semibold text-amber-800 flex items-center gap-2">
              ⚠️ App-generated cert exists ({existingCert.serialCode}) · v{existingCert.version} · Issued {fmtDate(existingCert.issuedAt)}.
              Submitting will reissue as <strong>v{existingCert.version + 1}</strong>.
            </div>
          )}

          {/* No app cert yet — informational */}
          {form.petId && !existingCert && (
            <div className="p-3.5 bg-blue-50 border border-blue-200 rounded-xl text-xs font-semibold text-blue-700 flex items-center gap-2">
              ℹ️ No app-generated certificate yet. This will create v1 — <strong>CERT-{form.petId}</strong>.
            </div>
          )}
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              placeholder="A personal message for the adopter…"
          ...
          {form.petName && (
            <div className="bg-[#fdf4e3] border-2 border-primary/30 rounded-2xl p-5 text-center">
              ...
              {/* ✅ Shows real serial code derived from petId */}
              <div className="text-[10px] text-primary font-mono mt-2">
                {form.petId ? `CERT-${form.petId}` : 'CERT-…'} · {new Date().toLocaleDateString()}
                {existingCert && (
                  <span className="ml-2 text-amber-600 font-bold">→ v{existingCert.version + 1}</span>
                )}
              </div>
            </div>
          )}
          ...
          <div className="flex gap-3 justify-end pt-2">
            <button
              onClick={() => { setIssueModal(false); setExistingCert(null); }}
              className="px-5 py-2 rounded-full border border-[#f0e8d5] text-sm font-bold">
              Cancel
            </button>
            <button
              onClick={handleIssue}
              disabled={busy || !form.petName}
              ...
              {busy
                ? 'Issuing…'
                : existingCert
                  ? `🔄 Reissue Certificate (v${existingCert.version + 1})`
