...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    await adminDb().collection('pets').doc(params.id).update({
      isDeleted: true, adminRemoved: true, adminRemovedReason: reason,
      adminRemovedBy: admin.uid, adminRemovedAt: FieldValue.serverTimestamp(),
      deletedAt: FieldValue.serverTimestamp(), status: 'removed',
    });
      ...
      try {
        ...
        if (owner?.email) {
          await emailService.petRemoved(
            owner.email, owner.name || 'Pet Owner', pet.name || 'Your pet', reason
          );
        }
      } catch (emailErr) {
